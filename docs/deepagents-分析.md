...
| **Python 中心** | myteams 等 TS 项目需桥接或选用 deepagentsjs |
...
## 7. 与 myteams 多 Agent 团队编排的相关性

### 7.1 myteams 现状（对照）

myteams 当前模型（`packages/core/src/types.ts`）：

| 概念 | myteams | Deep Agents |
|------|---------|-------------|
| 组织单元 | `Team` + `Member`（agent/human） | 单主 Agent + SubAgent 列表 |
| 任务 | `Mission` + `Phase`（brainstorm/scheme/delivery） | TodoList + 用户 prompt |
| 协作模式 | `CollaborationMode`（thread/workflow/hybrid） | 主线程 + `task` 委托 |
| 记忆 | `BriefingStore`（principles/patterns/scars） | `AGENTS.md` + MemoryMiddleware |
| 审计 | `LedgerStore`（message/decision/artifact/…） | LangSmith traces + conversation_history offload |
| 自治 | `AutonomyLevel` L0–L3 | `interrupt_on` + permissions |
| 引擎 | `HarnessConfig.engine`（当前 pi + stub） | `create_deep_agent` 单一栈 |

### 7.2 可借鉴的设计

```mermaid
flowchart LR
  subgraph myteams["myteams 团队层（待建设）"]
    T["Team YAML"]
    M["Mission / Phase"]
    L["Ledger"]
    B["Briefing"]
...
| Harness 适配 | 可作 `EngineAdapter` 新 engine=`deepagents` | Python 进程桥接或 deepagentsjs |

### 7.3 集成路径建议

1. **Engine 适配器**：在 `packages/harness` 注册 `deepagents` engine，将 `ContextBuilder` 输出映射为 `system_prompt.prefix` + `memory` 路径
2. **成员 → SubAgent**：`teams/*/members/*.md` 转为 `SubAgent.system_prompt`；`team.yaml` 生成 `subagents` 数组
3. **阶段 → 外层编排**：myteams 维护 Phase 状态机；每阶段 invoke 不同 prompt/skills 子集（Deep Agents 不管阶段，适合作为 **L1 执行层**）
4. **Ledger 互补**：Deep Agents 负责单成员执行与工具；myteams `LedgerStore` 负责跨成员可见性与审计
5. **async-subagent-server 示例**：远程成员可作为 `AsyncSubAgent` 部署

### 7.4 在横向对比矩阵中的位置

Deep Agents 应归类为 **L1+ Agent Harness**（比 Pi 更 opinionated，比 Omnigent/Symphony 更偏单 Agent 执行），与 Pi、LangChain `create_agent` 同层，**不具备 L4 团队协作语义**。myteams 宜将其视为可选 **成员运行时引擎**，团队编排语义仍由 myteams core 承担。
...
对 **myteams** 而言，Deep Agents 适合作为**可选成员执行引擎**（尤其需要子任务委托、文件工具链、长上下文管理的角色），团队章程、阶段状态机、账本与人类协作语义仍需由 myteams 上层实现；与 Pi（极简 TS harness）形成互补——Pi 轻量本地优先，Deep Agents 厚重、Python/LangGraph 生产链完整。