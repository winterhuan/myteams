...
| 维度 | CCG | myteams |
|------|-----|----------|
| 层级 | **Harness 内插件** | **独立协作平台** |
| 状态 | 文件 + hook 注入 | Hub DB + Thread/Custody |
| 多 Agent | Claude Agent Teams API | 跨引擎 Member + invoke |
| 质量门 | verify-* skills | Policy + workflow 节点 |
...
**不应做**：把 myteams 做成「只有 Claude Code 的 CCG」——那是 Harness 插件赛道。
