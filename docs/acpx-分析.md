...
若 myteams 需要编辑器/IDE 级 ACP 兼容，acpx 更合适；若需要 Pi 全量 RPC 事件（extension_ui 等），Paseo 路线更完整。
...
2. **中期**：用 Flows 编码 myteams 的多步 playbook（实现→review→merge gate）
3. **长期**：myteams 调度层嵌入 `acpx/runtime` 或 spawn `acpx` 子进程，避免自建 session/queue
