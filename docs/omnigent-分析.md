...
```
references/pi  ←── pi-native / pi harness ──→  references/omnigent
       │                                              │
       └──────────── references/opencode ──────────────┘
```

| myteams 参考项目 | Omnigent 中的集成点 |
|------------------|-------------------|
| `references/pi` | `pi` SDK harness + `pi-native` TUI harness |
| `references/opencode` | `opencode-native` harness |
| `references/codex` | `codex` + `codex-native` harness |
...
4. **SDK/API 契约**：`openapi.json` + `omnigent_client` 可作为 myteams 对外 API 的设计参考
...
2. **中期**：参考 Bridge 模式，为 myteams 实现独立的 Pi Web 桥接层
