...
OpenTeams 是 **「Agent Teams 产品」最接近的完整参考之一**：

| 能力 | myteams 应对齐 |
|------|-----------------|
| **双模式** | Thread（轻）+ WorkflowRun（重） |
| **共享 session 上下文** | Workspace 级 context 服务 |
| **步骤级控制** | 比 Clowder 球权更贴近用户心智 |
| **Preset Team** | Member 编组模板 |
| **可见计划图** | Hub UI 工作流视图 |

| 差异 |
|------|
| OpenTeams 偏 **单 session 多 agent 聊天 + 图** |
| 未必有 Clowder 级 custody 事件溯源 |
| Rust/Tauri 全栈重量大 |

**结论**：myteams 产品形态应 **更接近 OpenTeams + Multica 工作对象**，而非 **仅 Clowder 式 @+球权**。球权是 Thread 模式子机制；Workflow 是平行一等模式。
