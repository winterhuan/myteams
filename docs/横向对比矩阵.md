...
> 对比对象：`reference/` 下已拆解的参考实体（含 LangChain 生态第三批）  
> 更新日期：2026-07-06  
> 详细分析见：`docs/*-分析.md`
...
## 12. 第三批参考项目速览（LangChain 生态栈）

> 本批为 `reference/` 新增克隆：`langgraph`、`deepagents`、`openwiki`、`agno`。它们构成 LangChain 生态的 **L0 运行时 → L1 Harness → L3 平台 → 垂直工具** 纵向栈。

### 12.0 生态分层图

```mermaid
flowchart TB
  subgraph L3["L3 Agent 平台"]
    Agno["Agno\nAgentOS + Team + Workflow"]
  end

  subgraph L1plus["L1+ Harness"]
    DeepAgents["Deep Agents\n规划/子代理/文件系统/Skills"]
    OpenWiki["OpenWiki\n代码库 Wiki CLI"]
  end

  subgraph L0["L0 图运行时"]
    LangGraph["LangGraph\nPregel + Checkpoint + HITL"]
  end

  Agno -->|"可包裹"| LangGraph
  DeepAgents --> LangGraph
  OpenWiki --> DeepAgents
  Agno -->|"BaseExternalAgent"| LangGraph
```

### 12.0.1 总览矩阵

| 维度 | **LangGraph** | **Deep Agents** | **OpenWiki** | **Agno** |
|------|---------------|-----------------|--------------|----------|
| **层次** | L0 图运行时 | L1+ Harness | L1 垂直工具 | L3 Agent 平台 |
| **一句话** | 有状态 Agent 低层编排框架 | 开箱即用长任务 Agent Harness | Coding Agent 代码库文档 CLI | Python Agent 平台 SDK |
| **语言** | Python（JS 独立仓库） | Python | TypeScript (Node 20+) | Python |
| **成熟度** | Production/Stable | SDK Beta（0.6.x） | Early Alpha（0.0.1） | Production/Stable（2.6.x） |
| **开源协议** | MIT | MIT | MIT | Apache 2.0 |
| **维护方** | LangChain Inc. | LangChain Inc. | LangChain Inc. | agno.com |

### 12.0.2 关键能力对照

| 维度 | LangGraph | Deep Agents | OpenWiki | Agno |
|------|-----------|-------------|----------|------|
| **一等对象** | Graph/Node/Channel/Checkpoint | DeepAgentState/SubAgent/Backend | openwiki/ 目录 + UpdateMetadata | Agent/Team/Workflow/AgentOS |
| **多 Agent** | Send/Subgraph/Supervisor 模式 | `task` 子代理（主从委托） | 子 Agent 只读调研 + 主 Agent 写入 | TeamMode 四分法 + Workflow |
| **持久化** | Checkpoint + Store（KV/向量） | LangGraph checkpoint + DeltaChannel | SQLite thread + gitHead 快照 | BaseDb 多表 + LearningMachine |
| **HITL** | interrupt/Command 原生 | interrupt_on + 文件权限 | 无（全自动文档） | HumanReview + approvals router |
| **Harness 中立** | 中（langchain-core Runnable） | 低（LangChain 深度绑定） | 低（Deep Agents 绑定） | 中（自有 Model + 外部适配器） |
| **协作语义** | 图边/子图（无 Team 概念） | 主从 task 委托 | 无 | Team/Workflow/Session 完整 |
| **UI/控制面** | 无（LangSmith/Server 商业） | CLI/Talon（Alpha） | Ink TUI CLI | AgentOS FastAPI + 多通道 |
| **团队养成** | 无 | 无 | 无 | LearningMachine（偏用户记忆） |

### 12.0.3 与 myteams 的借鉴优先级

| myteams 能力 | 首选借鉴 | 次选 | 说明 |
|---------------|----------|------|------|
| **WorkflowRun 持久执行** | **LangGraph** | Agno Workflow | Checkpoint + interrupt 审批 |
| **单 Member 长任务执行** | **Deep Agents** | Pi harness | 子代理/文件系统/上下文摘要 |
| **代码库知识沉淀** | **OpenWiki** | — | `openwiki/` 约定 + AGENTS.md 注入 + Git 增量 |
| **Team 编排 + 生产 API** | **Agno** | OpenTeams | TeamMode、RunEvent、AgentOS 参考 |
| **Briefing/Ledger** | 无直接对应 | Agno LearningMachine | 需 myteams 自建领域模型 |

**结论**：LangChain 生态栈是 **执行与 Harness 参考**，不是 myteams 协作平台替代品。LangGraph/Deep Agents 适合作为 `EngineAdapter` 候选；Agno 适合作为 Team/Workflow 编排语义参考；OpenWiki 适合作为 Mission 知识层插件模式。

---

## 13. 第二批参考项目速览（Agent Teams 赛道）
...
| **执行多引擎** | acpx + **EngineAdapter** | Multica daemon / **Deep Agents** / LangGraph | 绑 Pi |
| **代码库 Wiki** | **OpenWiki** 模式 | — | 手写文档 |
| **Workflow 持久化** | **LangGraph** checkpoint | Agno Workflow | 无状态脚本 |
| **Team 生产 API** | **Agno** AgentOS 参考 | OpenTeams | 纯 CLI |
...
**myteams v0.7 立场**：参考矩阵用于**避坑与验证假设**，不用于拼功能。产品自有模型见 [myteams-方案.md](./myteams-方案.md)（Team / Mission / Ledger / Engagement / Briefing / Board）。
...
| **LangGraph** | 高 | Python 中心；LangChain 引力；低层需自建上层 |
| **Deep Agents** | 中高 | LangChain 绑定；无团队协作语义 |
| **OpenWiki** | 低 | 0.0.1 Alpha；Prompt/API 变动风险 |
| **Agno** | 高 | Python 单体；配置即代码；与 TS harness 栈需桥接 |
...
| LangGraph | [langgraph-分析.md](./langgraph-分析.md) |
| Deep Agents | [deepagents-分析.md](./deepagents-分析.md) |
| OpenWiki | [openwiki-分析.md](./openwiki-分析.md) |
| Agno | [agno-分析.md](./agno-分析.md) |

---

*本矩阵用于 myteams 架构决策；v0.7 起新增 LangChain 生态栈参考（L0 运行时 / L1 Harness / L3 平台），默认 **Harness 中立 + 双模式协作平台**，Pi 为默认引擎之一而非产品中心。*