...
| 局限 | 说明 |
|------|------|
| **Python 单体** | 与 TypeScript/Node 生态（如 myteams 的 Bun harness）集成需桥接 |
...
```text
myteams = 养多支专业 Agent 团队的平台
  ├── 领域团队（短剧/小说/应用开发）长期存在
  ├── 共性节奏：brainstorm → scheme → delivery
  ├── 各队自定义工作方式（team.yaml + phases/*.yaml）
  └── Harness 无关（Pi 等引擎）+ 团队记忆驱动进化
```

### 7.2 映射关系

| myteams 概念 | Agno 对应 | 匹配度 |
|--------------|-----------|--------|
...
1. **TeamMode 四分法** — myteams `defaultMode: hybrid` 可映射为 coordinate + workflow 阶段切换
2. **tasks 模式任务板** — 类似应用开发队 delivery 阶段的任务拆分与并行
3. **RunEvent 流** — 驱动 myteams 队内「共享现场」UI
...
| 差异 | 建议 |
|------|------|
| **语言栈** | Agno 作 Python 侧编排服务，myteams TS core 通过 HTTP/A2A 调用 AgentOS |
| **声明式 team.yaml** | 不直接采用 Agno；可参考其 Team/Workflow 语义编写 myteams 编排器 |
| **Harness 无关** | 用 `BaseExternalAgent` 模式包装 Pi RPC，或仅用 Agno 的 Team 编排层 |
| **领域阶段** | myteams `phases/*.yaml` 用 Workflow Step 序列实现，但阶段语义保留在 myteams |
| **团队进化** | 借鉴 `LearningMachine` 多 Store 架构，在 myteams `ledger-store` 落地 |

### 7.5 定位对比

```text
Agno     → 「Agent 平台基础设施」：如何跑、存、管 Agent
myteams  → 「专业编队养成平台」：养什么队、怎么协作、如何进化

关系：Agno 可作为 myteams 的 L3 编排/runtime 参考实现之一，
      但不适合作为 myteams 的直接技术底座（语言、harness 模型不匹配）。
```
...
**总结**：Agno 是目前参考库中 **最完整的 Python Agent 平台 SDK**，在 Team 协作、Workflow 编排、生产运行时三方面成熟度极高。对 myteams 而言，它是 **多 Agent 编排与生产运行时的最佳参考样本**，尤其在 TeamMode、事件流、HITL、记忆学习、组件持久化上值得深度借鉴；但由于 **语言栈与 Harness 模型差异**，更适合作为 **架构与语义参考**，而非直接 fork 为 myteams 底座。