...
```text
L4 工作与协作语义    ← myteams（多专业团队、brainstorm→方案→实施）
L3 编排与控制面
L2 多实例 / 协议桥接
L1 Agent 执行层      ← OpenWiki 所在层（单 Agent CLI + DeepAgents）
```

OpenWiki 属于 **L1 辅助能力**：为代码库（或 myteams 自身）生成 **Agent 可读的知识层**，而非团队编排引擎。

### 7.2 可借鉴的设计

| OpenWiki 机制 | myteams 可吸收点 |
|---------------|------------------|
...
| **CI 定时 PR 更新文档** | 应用开发队 delivery 后自动刷新 `docs/` 或队宪 |

### 7.3 与 myteams 草案的契合与差异

**契合**：

- myteams 强调 **队长期存在、自主进化** → OpenWiki 的 **持续 update + Git 驱动** 是「队知识进化」的一种实现范式
- 三支范例队（短剧/小说/应用开发）都需要 **结构化产物与记忆** → Wiki 是低成本「队外显记忆」
...
### 7.4 建议用法（在 myteams 项目中）

1. **为 myteams  monorepo 自身跑 OpenWiki**：生成 `openwiki/`，降低新 Agent 理解 `packages/core`、`teams/*` 的成本  
2. **应用开发队 delivery 流水线**：feature 合并后触发 `openwiki --update`，与 `teams/app-dev` 产物对齐  
3. **抽象「Context Builder」**：借鉴 `createRunContext` + Git 摘要，作为 myteams `context-builder.ts` 的参考实现  
...
OpenWiki 是 LangChain 生态下 **「Agent 文档工程师」** 的参考实现：用 DeepAgents + 极长 Prompt 把「读仓库、写 Wiki、接 AGENTS.md、跟 Git 更新」做成一条可本地、可 CI 的流水线。对 myteams 而言，它 **不是** 多 Agent 编排的竞品，而是 **队知识沉淀与上下文供给** 的高价值参考——尤其在 **增量更新、no-op、标准化 Agent 入口、子 Agent 只读并行** 四点，可直接翻译进平台的 Context/Memory 子系统。